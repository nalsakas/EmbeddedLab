...
uint8_t API_DATA[250];     ///< Data to send

/// @brief Formats and sends incoming massage to console.
/// @param packet Message to be showed.
void api_handle_packet(RPL3_PACKET *packet)
{
    if (packet->destination == API_BROADCAST)
    {
        uart_send_str("B ");
    }
    else
...
/// Status can be either BUSY or OK.
void api_status()
{
    uart_send_str("Data: ");
...
/// @brief This method send messages through layer 3. It uses rpl3_send() method in order to do so.
void api_push()
{
    if (!TX_BUSY)
    {
        rpl3_send(API_ADDRESS, API_BIND, API_DATA);
        uart_send_str("Y\n");
    }
    else
    {
...
void api_parse_data(uint8_t **line, uint8_t *data)
{
    ...
    if (**line != 0)
    {
        ...
        memcpy(data, start, MIN(end - start, 250));
        uart_send_str("Y\n");
        return;
    }
    uart_send_str("N\n");
...
/// This method send result of the actions to the console.
/// @param line Line to be parsed.
void api_handle_line(uint8_t *line)
{
    uint8_t API_COMMAND = '\0';
    api_parse_command(&line, &API_COMMAND);

    switch (API_COMMAND)
    {
    case 'A':
        api_parse_int(&line, &API_ADDRESS);
        break;
    case 'S':
        api_status();
        break;
    case 'D':
        api_parse_data(&line, API_DATA);
        break;
    case 'B':
        api_parse_int(&line, &API_BIND);
        break;
    case 'P':
        api_push();
        break;
